...
class RgbDepthTrainer(BaseTrainer):
    def compute_score(
        ...
        ground_truths: torch.Tensor,  # pyright: ignore unused
    ) -> float:
        ...
        metrics = {
            'rmse': (diff.square().sum() / n).sqrt().item(),
            'rel': (diff/torch.clamp_min(target, 1e-6))[mask_valid].mean().item(),
            'srel': (diff**2/torch.clamp_min(target, 1e-6))[mask_valid].mean().item(),
            'log10': (log_diff.square().sum() / n).sqrt().item(),
            'delta_1': (max_rel < 1.25).float().mean().item(),
            'delta_2': (max_rel < (1.25**2)).float().mean().item(),
            'delta_3': (max_rel < (1.25**3)).float().mean().item(),
        }
        return metrics
